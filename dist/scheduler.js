...
import { sendTeamsMessage, buildBugsReportedTodayCard, } from "./teamsNotifier.js";
...
export function startBugSummaryScheduler() {
    if (process.env.ENABLE_TEAMS_NOTIFICATIONS !== "true") {
        return;
    }
    const schedule = process.env.TEAMS_DAILY_SUMMARY_CRON ?? "45 17 * * 1-5";
    const timezone = process.env.TEAMS_SUMMARY_TIMEZONE ?? "Europe/Rome";
    cron.schedule(schedule, async () => {
        try {
            const records = await getDefectData();
            const bugsToday = records.filter((record) => isToday(record.createdDate, timezone));
            ...
            console.error("Failed to send daily bug report to Teams", error);
        }
    }, { timezone });
}
export function startSprintReportFileExportScheduler() {
    const destinationFolder = process.env.SPRINT_REPORT_EXPORT_PATH;
    if (process.env.ENABLE_SPRINT_REPORT_EXPORT !== "true" ||
        !destinationFolder) {
        return;
    }
    const schedule = process.env.SPRINT_REPORT_EXPORT_CRON ?? "0 18 * * *";
    const timezone = process.env.SPRINT_REPORT_EXPORT_TIMEZONE ?? "Europe/Rome";
    cron.schedule(schedule, async () => {
        try {
            const records = await getDefectData();
            const report = computeSprintDefectReport(records);
            const filePath = writeSprintDefectReportFile(report, destinationFolder);
            console.log(`Sprint Defect Report written to ${filePath}`);
        }
        catch (error) {
            console.error("Failed to write daily Sprint Defect Report file", error);
        }
    }, { timezone });
}
