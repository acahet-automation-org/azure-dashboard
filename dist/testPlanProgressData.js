import { getTestSuiteHierarchy, getTestSuiteCurrentCounts, getSuites, getTestCases, getTestPoints, } from "./azdo.js";
import { buildTestCaseRow, resolveTestPointStatus } from "./dashboardData.js";
const cache = new Map();
const CACHE_DURATION_MS = 5 * 60 * 1000;
export function clearTestPlanProgressCache() {
    cache.clear();
}
function zeroCounts() {
    return {
...
export async function computeTestPlanProgress(planId) {
...
export function clearTestPlanProgressBugsCache() {
    bugsCache.clear();
}
...
// getTestPoints below.
export async function computeTestPlanProgressBugs(planId, suiteIds) {
    ...
    const targetSuiteIds = normalizedSuiteIds.length
        ? normalizedSuiteIds
        : (await getSuites(planId)).map((suite) => suite.id);
    const bugsById = new Map();
    await Promise.all(targetSuiteIds.map(async (suiteId) => {
        ...
        const rows = await Promise.all(testCases.map((tc) => buildTestCaseRow(tc, "", "", suiteId, outcomesByTestCase, lastRunByTestCase)));
