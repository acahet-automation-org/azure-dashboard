import { getDashboardData, computeDashboardStats, computeExecutionTrend, } from "./dashboardData.js";
import { getDefectData } from "./defectData.js";
import { getCurrentSprint, getPreviousSprint } from "./sprints.js";
import { getIterations } from "./azdo.js";
...
const cache = new Map();
export function clearReleaseReadinessCache() {
    cache.clear();
}
...
function isFunctionalTestPlan(planName) {
...
function buildGateCriteria(completion, testsPassedPct, testCaseRelevancePct, severityOpenCounts) {
    const testsExecutedTarget = envNumber("RELEASE_GATE_TESTS_EXECUTED_TARGET_PCT", 100);
    const testsPassedTarget = envNumber("RELEASE_GATE_TESTS_PASSED_TARGET_PCT", 95);
    const requirementsCoverageTarget = envNumber("RELEASE_GATE_REQUIREMENTS_COVERAGE_TARGET_PCT", 95);
    const testCaseRelevanceTarget = envNumber("RELEASE_GATE_TEST_CASE_RELEVANCE_TARGET_PCT", 0);
    return [
        {
            id: "testsExecuted",
            action: "block",
        ...
        },
        {
            ...
            passed: testCaseRelevancePct != null &&
...
function computeReleaseGate(criteria) {
    const trackedCriteria = criteria.filter((c) => c.tracked);
    const hasBlockFailure = trackedCriteria.some((c) => c.action === "block" && !c.passed);
    const hasWarnFailure = trackedCriteria.some((c) => c.action === "warn" && !c.passed);
    return {
...
function sprintPassRateFromTrend(trend, startDate, endDate) {
    const inRange = trend.filter((p) => p.date >= startDate && p.date <= endDate);
    const totalExecuted = inRange.reduce((sum, p) => sum + p.passed + p.failed + p.blocked, 0);
...
function computePassRateDelta(trend, sprint, previousSprint) {
    const currentSprintPassRate = sprintPassRateFromTrend(trend, sprint.startDate, sprint.endDate);
    const previousSprintPassRate = previousSprint
        ? sprintPassRateFromTrend(trend, previousSprint.startDate, previousSprint.endDate)
        : null;
    return {
        currentSprintPassRate,
        previousSprintPassRate,
        deltaPct: currentSprintPassRate != null && previousSprintPassRate != null
            ? Math.round((currentSprintPassRate - previousSprintPassRate) * 10) / 10
            : null,
...
function scopedPassRatePct(testCases) {
...
function buildScopedPassRateDelta(currentSprintPassRate, previousSprint, functionalTestCases) {
    const previousSprintPassRate = previousSprint && typeof previousSprint.id === "string"
        ? scopedPassRatePct(functionalTestCases.filter((tc) => tc.iteration === previousSprint.id))
        : null;
    return {
        currentSprintPassRate,
        previousSprintPassRate,
...
// view below.
function sprintInfoFromIterationNode(node) {
...
function findPreviousIterationNode(nodes, node) {
    const dated = nodes
...
function resolveSprint(iteration, iterationNodes) {
    if (!iteration) {
...
export function countOpenBySeverity(defects) {
...
export async function computeReleaseReadiness(iteration) {
    const now = Date.now();
    ...
    const functionalTestCases = allTestCases.filter((tc) => isFunctionalTestPlan(tc.planName));
    ...
    const executedCount = dashboardStats.executedCount + dashboardStats.notApplicableCount;
    ...
    const testCaseRelevancePct = dashboardStats.totalTestCases
        ? Math.round((dashboardStats.notApplicableCount /
    ...
    const completion = {
        ...
        completionRatePct: dashboardStats.totalTestCases
            ? Math.round((executedCount / dashboardStats.totalTestCases) * 1000) / 10
            : 0,
    ...
    const testsPassedPct = dashboardStats.executedCount
        ? Math.round((dashboardStats.passedCount / dashboardStats.executedCount) *
            1000) / 10
        : null;
    const severityOpenCounts = countOpenBySeverity(scopedDefects);
    const criteria = buildGateCriteria(completion, testsPassedPct, testCaseRelevancePct, severityOpenCounts);
    ...
    const passRateDelta = iteration
        ? buildScopedPassRateDelta(testsPassedPct, previousSprint, functionalTestCases)
        : computePassRateDelta(trend, sprint, previousSprint);
    const blockingRecords = scopedDefects.filter((d) => d.state !== "Closed" &&
