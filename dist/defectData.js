import { getAllBugFields, getWorkItemRevisions, getStoryCount, getStoriesWithFields, getWorkItem, getWorkItems, extractWorkItemIds, buildWorkItemUrl, } from "./azdo.js";
import { getDashboardData } from "./dashboardData.js";
let defectCache = null;
...
function countReopenings(revisions) {
    let count = 0;
...
// that's decided.
function computeClosureReason(state, reason, tags) {
...
async function getLinkedTestCaseIds(bugId) {
    ...
    const testedByRelations = (workItem.relations ?? []).filter((r) => typeof r.rel === "string" &&
        r.rel.startsWith("Microsoft.VSTS.Common.TestedBy"));
    const linkedIds = extractWorkItemIds(testedByRelations);
...
async function getTestCaseLookups() {
    const iterationByTestCase = new Map();
    const titleByTestCase = new Map();
    const suiteByTitle = new Map();
    const testCases = await getDashboardData();
    for (const tc of testCases) {
        if (tc.iteration && !iterationByTestCase.has(tc.testCaseId)) {
            iterationByTestCase.set(tc.testCaseId, tc.iteration);
        }
...
function isSpecificIterationPath(path) {
    return typeof path === "string" && path.includes("\\");
}
async function buildDefectRecord(bug, lookups) {
    const [revisions, linkedTestCaseIds] = await Promise.all([
    ...
    // SUITE_NAME_ALIASES).
    const suiteName = normalizeSuiteName(bug.fields["Custom.Suite"]);
    ...
    let iterationPath = isSpecificIterationPath(bug.fields["System.IterationPath"])
...
export async function buildDefectRecords() {
    ...
    return Promise.all(bugs.map((bug) => buildDefectRecord(bug, lookups)));
}
...
function computeByTestSuite(records, allSuiteNames) {
    const result = {};
...
function computeTrend(records) {
    ...
    for (const record of records) {
        const createdWeek = weekStart(new Date(record.createdDate));
        const createdBucket = buckets.get(createdWeek) ?? { opened: 0, closed: 0 };
        createdBucket.opened++;
        buckets.set(createdWeek, createdBucket);
        if (record.closedDate) {
            const closedWeek = weekStart(new Date(record.closedDate));
            const closedBucket = buckets.get(closedWeek) ?? { opened: 0, closed: 0 };
            closedBucket.closed++;
...
function computeMttrDays(records) {
    ...
    const total = resolutionTimes.reduce((sum, days) => sum + days, 0);
    return (Math.round((total / resolutionTimes.length) * 10) / 10);
}
function computeAgingBuckets(records) {
    const now = Date.now();
    const bucketDefs = [
    ...
    for (const record of records) {
        ...
        const ageDays = (now -
            new Date(record.createdDate).getTime()) /
            (1000 * 60 * 60 * 24);
        const index = bucketDefs.findIndex((def) => ageDays >= def.min &&
            ageDays <= def.max);
        if (index >= 0) {
            counts[index].count++;
        }
...
function computeReopenDistribution(records) {
    const bucketDefs = [
    ...
    for (const record of records) {
        ...
        const index = bucketDefs.findIndex((def) => record.reopenedCount >= def.min &&
...
function computeRejectionRate(records) {
    if (records.length === 0) {
        return 0;
    }
    const rejectedCount = records.filter((r) => r.reason && REJECTED_REASONS.has(r.reason)).length;
    return (Math.round((rejectedCount / records.length) * 1000) / 10);
}
function computeRegressionRate(records) {
    if (records.length === 0) {
        return 0;
    }
    const regressionCount = records.filter((r) => r.tags.includes(REGRESSION_TAG)).length;
    return (Math.round((regressionCount / records.length) * 1000) / 10);
}
function computeRejectionReasons(records) {
    return groupCount(records.filter((r) => r.reason && REJECTED_REASONS.has(r.reason)), (r) => r.reason);
}
function computeClosureReasonBreakdown(records) {
    return groupCount(records.filter((r) => r.closureReason), (r) => r.closureReason);
}
function computeOutOfScopeRate(records) {
    if (records.length === 0) {
        return 0;
    }
    const outOfScopeCount = records.filter((r) => r.closureReason === "OutOfScope").length;
    return (Math.round((outOfScopeCount / records.length) * 1000) / 10);
}
function computeOutOfScopeBySuite(records) {
    return groupCount(records.filter((r) => r.closureReason === "OutOfScope"), (r) => r.suiteName);
}
...
function realTestFactorySuites(allSuiteNames) {
    return allSuiteNames.filter((suite) => suite !== DSI_SUITE_NAME && !(suite in DUPLICATE_SUITE_ORIGINS));
}
...
// every suite.
function computeTestFactoryBySuite(effectiveRecords, allSuiteNames) {
    const result = {};
...
function computeDuplicateSuiteBySuite(effectiveRecords, allSuiteNames, duplicateSuiteName) {
    const result = {};
    for (const suite of realTestFactorySuites(allSuiteNames)) {
        result[suite] = 0;
    }
...
export function computeSprintDefectReport(records, allSuiteNames = []) {
    ...
    return {
        ...
        testFactoryBySuite: computeTestFactoryBySuite(effective, allSuiteNames),
        testAgentiBySuite: computeDuplicateSuiteBySuite(effective, allSuiteNames, "Test Agenti"),
        testBusinessBySuite: computeDuplicateSuiteBySuite(effective, allSuiteNames, "Test Business"),
...
function computeFirstTimeFixRate(records) {
    const resolved = records.filter((r) => r.closedDate);
    if (resolved.length === 0) {
        return null;
    }
    const fixedFirstTry = resolved.filter((r) => r.reopenedCount === 0).length;
    return (Math.round((fixedFirstTry / resolved.length) * 1000) / 10);
}
function computeLeakageRate(records) {
    const withEnvironment = records.filter((r) => r.environment);
    if (withEnvironment.length === 0) {
        return null;
    }
    const foundInProd = withEnvironment.filter((r) => r.environment.toLowerCase().includes("prod")).length;
    return (Math.round((foundInProd / withEnvironment.length) * 1000) / 10);
}
function computeDensityByComponent(records, storyPointsByArea) {
    const bugCounts = groupCount(records, (r) => r.areaPath);
    const result = {};
    for (const [area, count] of Object.entries(bugCounts)) {
...
function computeSlaBreaches(records) {
    const now = Date.now();
    return records
...
function computeBacklogDirection(backlogTrend) {
    const recent = backlogTrend.slice(-4);
    if (recent.length === 0) {
        return "stable";
    }
    const avgDelta = recent.reduce((sum, p) => sum + p.delta, 0) /
        recent.length;
    if (avgDelta > 0.5) {
        return "growing";
    }
...
function computeStoryPointsByArea(stories) {
    const result = {};
...
function computeAvailableFilters(records, allSuiteNames = []) {
...
export function computeDefectStats(records, storyCount, storyPointsByArea = {}, allRecordsForFilterOptions = records, allSuiteNames = []) {
    const totalClosed = records.filter((r) => r.state === "Closed").length;
    const totalOpen = records.length - totalClosed;
    const duplicateCount = records.filter((r) => r.state === "Duplicate").length;
    const reopenedBugCount = records.filter((r) => r.reopenedCount > 0).length;
    ...
    return {
        totalOpen,
        totalClosed,
        bySeverity: groupCount(records, (r) => r.severity),
        byPriority: groupCount(records, (r) => r.priority != null
            ? String(r.priority)
            : undefined),
        byComponent: groupCount(records, (r) => r.areaPath),
        byTeam: groupCount(records, (r) => r.areaPath),
        byTestSuite: computeByTestSuite(records, allSuiteNames),
        byAssignee: groupCount(records, (r) => r.assignedTo?.displayName),
        trend,
        mttrDays: computeMttrDays(records),
        ...
        reopenRate: records.length
            ? Math.round((reopenedBugCount / records.length) *
                1000) / 10
            : 0,
        duplicateRate: records.length
            ? Math.round((duplicateCount / records.length) *
                1000) / 10
            : 0,
        bugsPerStory: storyCount
            ? Math.round((records.length / storyCount) * 100) / 100
            : null,
        ...
        densityByComponent: computeDensityByComponent(records, storyPointsByArea),
        backlogTrend,
        backlogDirection: computeBacklogDirection(backlogTrend),
        slaBreaches: computeSlaBreaches(records),
        availableFilters: computeAvailableFilters(allRecordsForFilterOptions, allSuiteNames),
    };
}
