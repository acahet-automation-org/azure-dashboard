...
async function sendViaGraph({ fromAddress, fromDisplayName, to, subject, bodyHtml, pdfBase64, filename, }) {
    const accessToken = await getGraphAccessToken();
    try {
        await axios.post(`https://graph.microsoft.com/v1.0/users/${encodeURIComponent(fromAddress)}/sendMail`, {
            message: {
                subject,
        ...
        if (axios.isAxiosError(error)) {
            ...
            throw new Error(`Microsoft Graph sendMail failed (HTTP ${error.response?.status}): ${graphError?.code ?? "unknown error"} - ${graphError?.message ?? error.message}`);
        }
        throw error;
    }
}
export async function sendReportEmail({ to, subject, bodyHtml, pdfBase64, filename, fromName, }) {
