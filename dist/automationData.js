import { getTestPlans, getSuites, getTestCases, getTestRuns, getTestRunResults, getWorkItems, } from "./azdo.js";
...
let rowsCache = null;
let occurrencesCache = null;
let cacheTimestamp = 0;
const CACHE_DURATION_MS = 5 * 60 * 1000;
export function getAutomationCacheTimestamp() {
...
async function buildAutomationRows() {
    ...
    for (const plan of plans) {
        ...
        for (const suite of suites) {
            ...
            const ids = testCases.map((tc) => tc.workItem.id);
            const workItems = await getWorkItems(ids, [
                AUTOMATION_STATUS_FIELD,
                AREA_PATH_FIELD,
            ]);
            const fieldsById = new Map(workItems.map((wi) => [
                wi.id,
                wi.fields,
            ]));
            for (const tc of testCases) {
                ...
                rows.push({
                    ...
                    isAutomated: fields[AUTOMATION_STATUS_FIELD] ===
                        "Automated",
                });
            }
        }
...
async function getAutomationData() {
    ...
    const { rows, occurrences } = await buildAutomationData();
    rowsCache = rows;
...
export function computeAutomationDashboard(rows, occurrences, planId, iteration) {
    ...
    const rowsInIteration = iteration
        ...
        : rows;
    const filteredRows = planId == null
        ? rowsInIteration
        : rowsInIteration.filter((r) => r.planId === planId);
    const automatedPlanIds = [
        ...new Set(rowsInIteration
            .filter((r) => r.isAutomated)
            .map((r) => r.planId)),
    ].sort((a, b) => a - b);
    const automatedRows = filteredRows.filter((r) => r.isAutomated);
    const automatedIds = new Set(automatedRows.map((r) => r.testCaseId));
    const automatedOccurrences = occurrences.filter((o) => automatedIds.has(o.testCaseId));
    const automatedTests = automatedRows.length;
    const manualTests = filteredRows.length - automatedTests;
    const automationCoveragePct = filteredRows.length
        ? Math.round((automatedTests / filteredRows.length) *
            1000) / 10
        : 0;
    const occurrencesByTestCase = new Map();
    for (const occurrence of automatedOccurrences) {
        const bucket = occurrencesByTestCase.get(occurrence.testCaseId) ?? [];
        bucket.push(occurrence);
        occurrencesByTestCase.set(occurrence.testCaseId, bucket);
    }
    const testCaseTitleById = new Map(automatedRows.map((r) => [
        r.testCaseId,
        r.testCaseTitle,
    ]));
    let totalPassed = 0;
    let totalFailed = 0;
    const flakyTestRanking = [];
    for (const [testCaseId, testOccurrences,] of occurrencesByTestCase) {
        const passed = testOccurrences.filter((o) => o.outcome === "Passed").length;
        const failed = testOccurrences.filter((o) => o.outcome === "Failed").length;
        totalPassed += passed;
        totalFailed += failed;
        if (passed > 0 && failed > 0) {
            ...
            flakyTestRanking.push({
                testCaseId,
                testName: testCaseTitleById.get(testCaseId) ??
                    `Test #${testCaseId}`,
                flakeCount: failed,
                lastFailedDate,
            });
    ...
    flakyTestRanking.sort((a, b) => b.flakeCount - a.flakeCount);
    const automationSuccessRatePct = totalPassed + totalFailed > 0
        ? Math.round((totalPassed /
    ...
    const charts = {
        coverageByModule,
        flakyTestRanking: flakyTestRanking.slice(0, FLAKY_TOP_N),
...
export async function getAutomationDashboard(planId, iteration) {
    const { rows, occurrences } = await getAutomationData();
    return {
        ...computeAutomationDashboard(rows, occurrences, planId, iteration),
