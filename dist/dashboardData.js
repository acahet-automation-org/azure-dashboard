import { getTestPlans, getSuites, getTestCases, getTestPoints, getTestRuns, getTestRunStatistics, getWorkItem, getWorkItems, extractWorkItemIds, buildWorkItemUrl, buildTestRunUrl, deleteTestCase, deleteTestCasesFromSuite, } from "./azdo.js";
...
export function resolveTestPointStatus(point) {
...
export function resolveOutcome(outcomes) {
...
export async function buildTestCaseRow(tc, planName, suiteName, suiteId, outcomesByTestCase, lastRunByTestCase, planIteration) {
    const workItem = await getWorkItem(tc.workItem.id);
    const linkedIds = extractWorkItemIds(workItem.relations);
    const linkedItems = await getWorkItems(linkedIds);
    const bugs = linkedItems.filter((item) => item.fields["System.WorkItemType"] === "Bug");
    const openBugs = bugs.filter((b) => b.fields["System.State"] !==
        "Closed");
    ...
    return {
        ...
        priority: workItem.fields["Microsoft.VSTS.Common.Priority"] ?? 4,
        hasOpenBugs: openBugs.length > 0,
        outcome: resolveOutcome(outcomesByTestCase[tc.workItem.id] ?? []),
        bugs: bugs.map((b) => ({
            id: b.id,
...
function indexSuiteTestPoints(testPoints) {
...
export async function buildDashboard() {
    ...
    for (const plan of plans) {
        ...
        for (const suite of suites) {
            const testCases = await getTestCases(plan.id, suite.id);
            const testPoints = await getTestPoints(plan.id, suite.id);
            const { outcomesByTestCase, lastRunByTestCase } = indexSuiteTestPoints(testPoints);
            const rows = await Promise.all(testCases.map((tc) => buildTestCaseRow(tc, plan.name, suite.name, suite.id, outcomesByTestCase, lastRunByTestCase, plan.iteration)));
...
export function computeDashboardStats(allTestCases) {
    const grouped = {};
    ...
    const areaPaths = [
        ...new Set(allTestCases
            .map((tc) => tc.areaPath)
            .filter(Boolean)),
    ].sort((a, b) => a.localeCompare(b));
    const suites = [
        ...new Set(allTestCases.map((tc) => tc.suiteName)),
    ].sort((a, b) => a.localeCompare(b));
    const priorities = [
        ...new Set(allTestCases.map((tc) => tc.priority)),
    ].sort((a, b) => a - b);
    const totalTestCases = allTestCases.length;
    const withOpenBugs = allTestCases.filter((tc) => tc.hasOpenBugs).length;
    const withoutOpenBugs = totalTestCases - withOpenBugs;
    const passedCount = allTestCases.filter((tc) => tc.outcome === "Passed").length;
    const failedCount = allTestCases.filter((tc) => tc.outcome === "Failed").length;
    const blockedCount = allTestCases.filter((tc) => tc.outcome === "Blocked").length;
    const notApplicableCount = allTestCases.filter((tc) => tc.outcome === "NotApplicable").length;
    const notRunCount = allTestCases.filter((tc) => tc.outcome === "NotRun").length;
    const executedCount = passedCount + failedCount + blockedCount;
    const passRate = totalTestCases
        ? Math.round((passedCount / totalTestCases) * 1000) / 10
        : 0;
    return {
        areaPaths,
        suites,
...
export function computeSuiteStats(allTestCases) {
    const suiteStats = {};
    for (const tc of allTestCases) {
...
function summarizeRunStats(stats) {
    const counts = {
        Passed: 0,
        Failed: 0,
        Blocked: 0,
    ...
    const total = counts.Passed +
        counts.Failed +
        counts.Blocked +
        counts.NotApplicable +
        counts.NotRun;
    const passRate = total
        ? Math.round((counts.Passed / total) * 1000) / 10
        : 0;
...
export async function computePlanSuites(planId) {
    const suites = await getSuites(planId);
    const testCasesBySuiteId = new Map(await Promise.all(suites.map(async (suite) => {
    ...
    const nodesById = new Map(suites.map((suite) => [
        suite.id,
        {
            id: suite.id,
            name: suite.name,
            testCases: testCasesBySuiteId.get(suite.id) ??
                [],
            children: [],
        },
    ]));
...
export async function computeRunCards() {
    ...
    const last10 = [...runs]
        ...
        const aDate = new Date(a.completedDate ??
            a.startedDate).getTime();
        const bDate = new Date(b.completedDate ??
            b.startedDate).getTime();
        return bDate - aDate;
    })
        .slice(0, 10);
    const cards = await Promise.all(last10.map(async (run) => {
        const stats = await getTestRunStatistics(run.id);
        ...
        const { counts, total, passRate } = summarizeRunStats(stats);
        return {
            id: run.id,
            name: run.name,
            state: run.state,
...
export async function computeExecutionTrend() {
    const runs = await getTestRuns();
    const runStatsWithNulls = await Promise.all(runs.map(async (run) => {
        const stats = await getTestRunStatistics(run.id);
    ...
    const runStats = runStatsWithNulls.filter((entry) => entry !== null);
    const byDate = new Map();
    for (const { run, counts } of runStats) {
        const rawDate = run.completedDate ?? run.startedDate;
        if (!rawDate) {
            continue;
        }
    ...
    return [...byDate.keys()]
        ...
        cumulativeExecuted +=
            bucket.passed +
                ...
                bucket.blocked;
        const passRate = total
            ? Math.round((bucket.passed / total) * 1000) / 10
            : 0;
        return {
            date,
            total,
...
export async function deleteTestCases(items) {
    ...
    const unlinkResults = await Promise.allSettled(groupList.map((group) => deleteTestCasesFromSuite(group.planId, group.suiteId, group.testCaseIds)));
    const deleted = [];
    const failed = [];
    ...
    await Promise.allSettled(deleted.map((id) => deleteTestCase(id)));
    if (deleted.length > 0) {
        clearDashboardCache();
    }
