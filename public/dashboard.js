...
const testcaseEls = Array.from(document.querySelectorAll(".testcase"));
const allSuiteOptions = Array.from(suiteFilter.options).slice(1);
const areaToSuites = {};
...
function updateSuiteOptions() {
    ...
    allSuiteOptions.forEach((opt) => {
        if (!allowedSuites ||
            ...
            suiteFilter.appendChild(opt.cloneNode(true));
        }
    });
    const stillValid = Array.from(suiteFilter.options).some((o) => o.value === currentSuite);
...
function updateGroupVisibility() {
    document
        .querySelectorAll(".suite-group")
        .forEach((group) => {
        const cards = Array.from(group.querySelectorAll(".testcase"));
        const visibleCount = cards.filter((card) => card.style.display !==
            "none").length;
        const countEl = group.querySelector(".suite-count");
        if (countEl) {
    ...
    document
        ...
        .forEach((p) => {
        const visible = Array.from(p.querySelectorAll(".suite-group")).some((group) => group.style.display !==
            "none");
...
function updateStats() {
    ...
    testcaseEls.forEach((card) => {
        ...
        total++;
        const active = Number(card.dataset.bugActive || 0);
        const closed = Number(card.dataset.bugClosed || 0);
        activeBugs += active;
        closedBugs += closed;
        if (active > 0) {
            withBugs++;
        ...
        else if (card.dataset.outcome === "Failed") {
            failed++;
        }
        else if (card.dataset.outcome === "Blocked") {
            blocked++;
        }
        else {
            notRun++;
    ...
    setText("statWithBugs", "🔴 " + withBugs);
    setText("statWithoutBugs", "✅ " + withoutBugs);
    setText("statActiveBugs", "🐞 " + activeBugs);
    setText("statClosedBugs", "✔ " + closedBugs);
...
function applyFilters() {
    ...
    testcaseEls.forEach((card) => {
        ...
        const areaMatch = !selectedArea ||
            area === selectedArea;
        const suiteMatch = !selectedSuite ||
            suite === selectedSuite;
